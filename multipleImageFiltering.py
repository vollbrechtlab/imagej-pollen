...
def folderSelect(title):
	od = DirectoryChooser(title)
...
def main():
	...
	inputFolder = folderSelect('Choose input folder')
	outputFolder = folderSelect('Choose output folder')
	
	if inputFolder != None and outputFolder != None:
		inputFiles = searchFolder(inputFolder, ['TIF', 'tif'])
		multipleImgFiltering(inputFolder, inputFiles, outputFolder)
		#print('done!')
		dialog = GenericDialog("Done!")
		dialog.showDialog()
	else:
		#print('failed!')
		dialog = GenericDialog("Failed!")
		dialog.showDialog()
